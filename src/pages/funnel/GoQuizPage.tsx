...
export default function GoQuizPage() {
  ...
  useEffect(() => {
    track('funnel_quiz_start', { content_name: 'go_quiz' })
  }, [])
  ...
  async function submitNurtureLead(e: FormEvent<HTMLFormElement>) {
    ...
    data.set('source', 'funnel_quiz_nurture')
    ...
    track('guide_form_submit', { location: 'funnel_quiz' })
      ...
      <section className="mx-auto max-w-lg px-5 py-10 sm:py-14">
        ...
        {phase === 'quiz' && question && (
          <>
            <p className="text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-gold-400">
              {funnelCopy.quiz.eyebrow} · {step + 1}/{quizQuestions.length}
            </p>
            {step === 0 && (
              <>
                <h1 className="mt-3 text-h2 font-bold text-white">{funnelCopy.quiz.headline}</h1>
                <p className="mt-3 text-body text-white/65">{funnelCopy.quiz.lede}</p>
              </>
            )}
            ...
            <Button
              ...
              onClick={() =>
                track('cta_book_click', {
                  location: 'funnel_quiz_qualified',
                  score: result.score,
                })
              }
            >
        ...
        {phase === 'result' && !result.qualified && (
          <div>
            ...
            <h1 className="mt-3 text-h2 font-bold text-white">{funnelCopy.nurture.headline}</h1>
            <p className="mt-3 text-body text-white/70">{funnelCopy.nurture.lede}</p>
              ...
              <div className="mt-8 rounded-card border border-white/15 bg-white/5 p-6">
                ...
                <p className="mt-2 text-small text-white/65">
                  ...
                  <Link
                    ...
                    onClick={() => track('cta_book_click', { location: 'funnel_quiz_nurture_success' })}
                  >
                    Book a triage call
                  </Link>
                  .
            ...
            <p className="mt-6 text-center text-small text-white/45">
              ...
              <Link
                ...
                onClick={() => track('cta_book_click', { location: 'funnel_quiz_nurture_alt' })}
              >
                Book a triage call
              </Link>
            </p>
